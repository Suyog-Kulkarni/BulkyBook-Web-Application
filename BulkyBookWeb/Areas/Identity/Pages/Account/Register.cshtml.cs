...
namespace BulkyBookWeb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IEmailSender _emailSender;// it does not have default implementation so we need to inject it 
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            /*if (!_roleManager.RoleExistsAsync(SD.Role_Customer).GetAwaiter().GetResult())
            {
                ...
                *//*await*//* _roleManager.CreateAsync(new IdentityRole(SD.Role_Admin)).GetAwaiter().GetResult();
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if(Input.Role is SD.Role_Company)
                {
                    user.CompanyId = Input.CompanyId;// this is the company id which we have selected in the dropdown list in view 
                }
                ...
                if (result.Succeeded)
                {
                    if (!String.IsNullOrEmpty(Input.Role))
                    {
                        ...
                        // this is the role which we have selected in the dropdown list in view 
                    }
                    else
                    {
                        await _userManager.AddToRoleAsync(user, SD.Role_Customer);
                        // if we have not selected any role then by default it will be customer role 
                    }
